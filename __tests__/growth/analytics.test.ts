...
describe("calculateExperimentResults", () => {
  ...
  it("reports lift, allocation balance, and no winner claim", () => {
    const results = calculateExperimentResults([
      exposure("c1", "control", undefined, "free_shipping_progress_v1"),
      exposure("t1", "treatment", undefined, "free_shipping_progress_v1"),
      exposure("t2", "treatment", undefined, "free_shipping_progress_v1"),
    ], [
      event("c1", "checkout_started"),
      event("t1", "checkout_started"),
      event("t2", "add_to_cart"),
    ]);
    const result = results.find(({ key }) => key === "free_shipping_progress_v1")!;
