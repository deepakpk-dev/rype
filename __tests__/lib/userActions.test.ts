...
const asAdmin = (email = "admin@rype.local") =>
  mockAuth.mockResolvedValue({ user: { role: "admin", email } } as never);
const asStaff = () =>
  mockAuth.mockResolvedValue({ user: { role: "staff", email: "staff@rype.local" } } as never);
...
describe("createUserAction", () => {
  ...
  it("normalizes the email and stores a bcrypt hash, never the password", async () => {
    ...
    expect(data.email).toBe("new.person@rype.local");
