...
const EXPERIMENT_LABELS: Record<ExperimentKey, string> = {
  ...
  free_shipping_progress_v1: "Free shipping progress",
...
export function GrowthDashboard({
  ...
  return (
    <div className="space-y-8">
      <header className="rounded-3xl border border-rype-line bg-white p-6 shadow-soft sm:p-8">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            <div className="mb-3 flex items-center gap-2 text-xs font-medium uppercase tracking-wider text-rype-leafDark">
            ...
            <p className="mt-2 text-sm leading-6 text-rype-mute">
          ...
          <div className="flex flex-wrap gap-2">
            <span className="inline-flex items-center gap-1.5 rounded-full border border-rype-line bg-rype-cream px-3 py-1.5 text-xs font-medium text-rype-ink">
              <CalendarDays className="h-3.5 w-3.5 text-rype-leafDark" />
              Last {data.window.days} days
            </span>
            {data.includesDemo && (
              <span className="inline-flex items-center gap-1.5 rounded-full bg-rype-yellow/25 px-3 py-1.5 text-xs font-medium text-rype-ink">
        ...
        <p className="mt-5 border-t border-rype-line pt-4 font-mono text-xs text-rype-mute">
          {windowDate(data.window.start)}{" \u2013 "}{windowDate(data.window.end)}{" \u00b7 rolling UTC window"}
        </p>
      </header>

      {dbError && (
        <section className="flex items-start gap-3 rounded-xl border border-rype-orange/30 bg-rype-orange/5 px-4 py-3 text-sm">
          <Database className="mt-0.5 h-4 w-4 shrink-0 text-rype-orange" />
          <div>
            <h2 className="font-medium text-rype-ink">Analytics database unavailable</h2>
            <p className="mt-0.5 text-rype-ink/70">{dbError}</p>
          </div>
        </section>
      )}
          ...
          {data.funnel.stages.map((stage, index) => (
            <article key={stage.name} className="card overflow-hidden p-5">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="font-mono text-[11px] text-rype-mute">
              ...
              <p className="mt-1 text-xs text-rype-mute">
                {index === 0 ? "Window baseline" : `${percent(stage.previousRate)} from previous stage`}
              </p>
              <div className="mt-4 h-1.5 overflow-hidden rounded-full bg-rype-line" aria-hidden="true">
                <div
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {data.acquisition.length === 0 ? (
                  <tr>
                    <td colSpan={3} className="px-5 py-10 text-center text-rype-mute">
                      No acquisition sessions in this window.
                    </td>
                  </tr>
                ) : (
                  data.acquisition.map((row) => (
                    <tr key={`${row.source}:${row.campaign ?? ""}`} className="border-t border-rype-line">
                      <td className="px-5 py-3 font-medium">{row.source}</td>
                      <td className="px-5 py-3 text-rype-mute">{row.campaign ?? "No campaign"}</td>
        ...
        <div className="mt-4 grid gap-5 xl:grid-cols-2">
          {data.experiments.map((experiment) => (
            <article key={experiment.key} className="card p-5 sm:p-6">
              <div className="flex flex-col gap-4 border-b border-rype-line pb-5 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    <Beaker className="h-4 w-4 text-rype-leafDark" />
                    <h3 className="font-display text-xl font-semibold">
                      {EXPERIMENT_LABELS[experiment.key]}
                    </h3>
                  </div>
                  <div className="mt-1 font-mono text-[11px] text-rype-mute">
                    {experiment.key}{" \u00b7 v"}{experiment.version}
                  </div>
                </div>
                <div className="rounded-lg bg-rype-leaf/10 px-3 py-2 text-xs text-rype-leafDark">
              ...
              <div className="grid gap-3 py-5 sm:grid-cols-2">
                {(["control", "treatment"] as const).map((variant) => {
                  ...
                  return (
                    <div key={variant} className="rounded-xl border border-rype-line bg-rype-cream/40 p-4">
                      <div className="text-xs font-semibold uppercase tracking-wide text-rype-mute">
                        {variant}
                      </div>
              ...
              <dl className="grid gap-3 border-t border-rype-line pt-5 sm:grid-cols-2">
                <div>
                  <dt className="text-xs text-rype-mute">{"Absolute lift (treatment \u2212 control)"}</dt>
                  <dd className="mt-1 font-mono text-sm font-semibold">
                    {percentagePoints(experiment.absoluteLift)}
                  </dd>
                </div>
                <div>
                  <dt className="text-xs text-rype-mute">Allocation balance (control / treatment)</dt>
                  <dd className="mt-1 font-mono text-sm font-semibold">
              ...
              <div className={`mt-5 flex items-start gap-2 rounded-xl px-3 py-2.5 text-xs ${
                experiment.evidence === "insufficient"
                  ? "bg-rype-yellow/20 text-rype-ink"
      ...
      <section className="rounded-2xl border border-rype-line bg-white p-5" aria-labelledby="limitations-heading">
        <div className="flex items-center gap-2">
          <Info className="h-4 w-4 text-rype-leafDark" />
          <h2 id="limitations-heading" className="font-display text-xl font-semibold">Limitations</h2>
        </div>
        <ul className="mt-3 grid gap-2 text-sm leading-6 text-rype-mute md:grid-cols-2">
...
function SectionHeading({
  ...
  return (
    <div>
      <div className="text-xs font-medium uppercase tracking-wider text-rype-mute">{eyebrow}</div>
      <h2 id={id} className="mt-1 font-display text-2xl font-semibold">{title}</h2>
      <p className="mt-1 text-sm text-rype-mute">{description}</p>
    </div>
  );
}
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-baseline justify-between gap-3">
      <dt className="text-rype-mute">{label}</dt>
