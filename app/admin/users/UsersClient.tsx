      ...
      {!dbError && (
        <form onSubmit={submitCreate} className="card mb-6 p-5">
          <div className="mb-3 flex items-center gap-2 font-medium">
            <UserPlus className="h-4 w-4 text-rype-leafDark" /> Add a user
          </div>
          ...
          {formError && <p className="mt-2 text-xs text-rype-red">{formError}</p>}
            ...
            >
              ...
              <div className="mt-4 flex items-center gap-2">
                <select
                  ...
                  className="rounded-full border border-rype-line bg-white px-2 py-0.5 text-xs capitalize text-rype-ink disabled:cursor-not-allowed disabled:text-rype-mute"
                  value={u.role}
                  disabled={isMe}
                ...
                <span className="flex-1 text-xs text-rype-mute">
                  {u.role === "admin"
                ...
                <button
                  ...
                  className="rounded-full p-1.5 text-rype-mute hover:bg-rype-red/10 hover:text-rype-red disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:bg-transparent disabled:hover:text-rype-mute"
                >
                  <Trash2 className="h-4 w-4" />
                </button>
              </div>
              {rowError?.id === u.id && (
                <p className="mt-2 text-xs text-rype-red">{rowError.message}</p>
              )}
            </div>
          );
        })}
