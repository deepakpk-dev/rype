...
const EXPERIMENT_ID_CODES: Record<ExperimentKey, string> = {
  ...
  free_shipping_progress_v1: "shipping",
...
export function buildGrowthDemoRows(anchorDate: Date): GrowthDemoRows {
  ...
  for (let index = 0; index < DEMO_SESSION_COUNT; index += 1) {
    ...
    const startedCheckout = addedToCart && deterministicScore(index, 2) < (
      assignments.free_shipping_progress_v1 === "treatment" ? 80 : 78
    );
    ...
    if (addedToCart) {
      ...
      expose("free_shipping_progress_v1", 6);
        ...
        if (completedCheckoutStep) {
          ...
          if (completedOrder) {
            event("order", "order_completed", 25, {
              ...
              cartValue: cartValue + (cartValue < 4000 ? 399 : 0),
              cartSize,
            });
          }
        }
